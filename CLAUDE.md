...
Tauri 2 桌面应用（Wind0ws_Dynamic_island）。Rust 后端 + Svelte 5 前端，通过 Tauri IPC 通信。包管理器 pnpm，构建工具 Vite。
视觉与交互基调：专为 Windows 系统打造的复刻 iOS 灵动岛桌面应用。严格遵循标准的 iOS 风格界面与频谱样式，追求极致的 120Hz 丝滑动画体验与物理弹簧反馈，同时要求极低的系统内存占用。
...
- **视觉风格锁定**：严禁尝试实现“液态玻璃（Liquid Glass）”等复杂特效，坚决执行并回归标准 iOS 风格的频谱与界面设计。
...
## AI 助手开发约束 (AI Prompt Directives)

### 必须做

1. **执行检查**：修改 Rust 后确保 `cargo check` + `cargo clippy` 通过；修改 Svelte 后确保 `pnpm check` 通过。
2. **完整 IPC 链路**：新增 Tauri command 时同步完成四步：
   - `commands/` 添加 command 函数
   - `main.rs` 注册到 `generate_handler!`
   - `$lib/api/` 添加封装函数
   - `$lib/types/` 更新类型
3. **数据结构同步**：修改数据模型时 Rust 和 TypeScript 同步修改，字段名保持一致（Rust 端确保保留 `camelCase` 宏）。
4. **最小化介入**：保持最小改动，只改与任务相关的代码。
5. **性能第一**：在生成 Rust 业务逻辑时，必须将“低内存占用”与“非阻塞主线程”作为首要考量。

### 禁止做

1. **禁用内联调用**：在 `.svelte` 中直接 `import { invoke }`。
2. **禁用危险解包**：在 command / service 层使用 `.unwrap()` / `.expect()`。
3. **拒绝对旧版语法妥协**：使用 Svelte 4 语法（`export let`、`$:`、`createEventDispatcher`）。
4. **拒绝硬编码**：硬编码颜色 / 间距 / 圆角，必须使用 CSS 变量。
5. **违背视觉基调**：擅自使用内凹/倒角圆角，或提议使用“液态玻璃”等不符合规范的特效。
6. **违背交互分离**：将控制类按钮（如关闭、设置）塞入 Dynamic Island 中。
7. **牺牲动画体验**：为了精简代码而牺牲 UI 动画渲染性能（导致跳帧、卡顿）。如果遇到性能瓶颈，首选 CSS `transform`/`opacity` 硬件加速及 Spring 物理参数微调。
8. **擅自越权**：未说明理由就修改 `tauri.conf.json` 权限或 `Cargo.toml` 依赖。
9. **过度设计**：做超出任务范围的架构变更。
10. **禁用标准输出日志**：使用 `println!` 代替 `tracing` 库。

### 不确定时

- **主动沟通**：先提出方案和取舍理由（特别是在权限、依赖增加等场景），确认后再实现。
- **提供视图**：IPC 接口变更时同时列出 Rust 和 TypeScript 两侧改动。
- **说明影响**：权限变更时详细说明其对应用的安全影响。
