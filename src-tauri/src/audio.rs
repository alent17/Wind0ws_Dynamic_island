        ...
        std::thread::spawn(move || {
            ...
            while running.load(Ordering::Acquire) && generation.load(Ordering::Acquire) == token {
                ...
                while running.load(Ordering::Acquire) && generation.load(Ordering::Acquire) == token
                {
                    ...
                    // WASAPI loopback streams stay bound to their original endpoint.
                    // Rebuild the stream when Windows changes the default output.
                    if host
                        .default_output_device()
                        .and_then(|current| current.name().ok())
                        .as_deref()
