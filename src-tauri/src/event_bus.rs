...
impl EventBus {
    ...
    pub fn initialize(&self, app: AppHandle) -> AppResult<()> {
        let mut handle = self
            .app_handle
            .lock()
            .map_err(|_| AppError::lock("Failed to lock app_handle"))?;
        *handle = Some(app);
        Ok(())
    }
    ...
    pub fn set_enabled(&self, enabled: bool) -> AppResult<()> {
        let mut e = self
            .enabled
            .lock()
            .map_err(|_| AppError::lock("Failed to lock enabled"))?;
        *e = enabled;
        Ok(())
    }
    ...
    ///
    /// 成功返回 `Ok(())`，失败返回错误信息
    pub fn emit<T: Serialize + Clone>(&self, event: &str, payload: T) -> AppResult<()> {
    ...
    pub fn emit_with_options<T: Serialize + Clone>(
        ...
        // 检查是否启用
        {
            let enabled = self
                .enabled
                .lock()
                .map_err(|_| AppError::lock("Lock failed"))?;
            if !*enabled {
                return Ok(());
            }
        }
        ...
        let app = {
            ...
            match handle.as_ref() {
                Some(app) => app.clone(),
                None => return Err(AppError::lock("AppHandle not initialized")),
            }
        };
        ...
        match app.emit(event, payload) {
            Ok(_) => {
                ...
                let mut stats = self
                    .stats
                    .lock()
                    .map_err(|_| AppError::lock("Failed to lock stats"))?;
                let entry = stats.entry(event.to_string()).or_default();
    ...
    /// 每个事件的发送结果列表
    #[allow(dead_code)]
    pub fn emit_batch<T: Serialize + Clone>(&self, events: Vec<(&str, T)>) -> Vec<AppResult<()>> {
        events
            .into_iter()
...
///
/// - `payload`: 媒体状态数据
pub fn emit_media_update<T: Serialize + Clone>(payload: T) -> AppResult<()> {
    EVENT_BUS.emit_with_options(
        EVENT_MEDIA_UPDATE,
        payload,
        Some(ThrottleConfig {
...
/// - `payload`: 频谱数据
#[allow(dead_code)]
pub fn emit_audio_spectrum<T: Serialize + Clone>(payload: T) -> AppResult<()> {
    EVENT_BUS.emit_with_options(
...
/// - `payload`: 设置数据
#[allow(dead_code)]
pub fn emit_settings_updated<T: Serialize + Clone>(payload: T) -> AppResult<()> {
...
#[allow(dead_code)]
pub fn emit_settings_changed(setting_name: &str) -> AppResult<()> {
