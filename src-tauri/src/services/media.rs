...
use crate::models::{MediaCapabilities, MediaState, NeteaseSong};
...
fn active_session() -> AppResult<Option<GlobalSystemMediaTransportControlsSession>> {
    ...
    let mut best = None;
    let mut best_key = (false, i64::MIN);
    for index in 0..sessions.Size().unwrap_or(0) {
        if let Ok(session) = sessions.GetAt(index) {
            ...
            let updated = session
                ...
                .unwrap_or(i64::MIN);
            if (playing, updated) > best_key {
                best_key = (playing, updated);
                best = Some(session);
            }
        }
    }
...
pub fn get_media_info(app: &AppHandle) -> AppResult<MediaState> {
    ...
    let mut best_session: Option<(GlobalSystemMediaTransportControlsSession, String, String)> =
        None;
    let mut best_key = (false, i64::MIN);
...
/// - "next": 下一曲
/// - "prev": 上一曲
pub fn control_media(action: &str) -> AppResult<()> {
    // 初始化 COM
    unsafe {
