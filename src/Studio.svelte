<script lang="ts">
  ...
  import { Play,Pause,ImageOff,Type,RotateCcw,Trash2,ExternalLink,Monitor } from "lucide-svelte";
  import LegacyIslandPreview, { type LegacyIslandMode } from "$lib/LegacyIslandPreview.svelte";
  ...
  let mode=$state<LegacyIslandMode>("expanded"); let scenario=$state<Scenario>("playing"); let progress=$state(50); let settings=$state<AppPreferences|null>(null); let monitors=$state<MonitorInfo[]>([]); let cacheMessage=$state("");
...
<main>
  ...
  <div class="workspace">
    <section class="stage" aria-label="灵动岛白底预览">
      <div class="ruler top"><span>0</span><span>300</span></div>
      <LegacyIslandPreview media={sample} {mode} position={sample.positionMs} onPreviewPlayPause={() => scenario = scenario === "paused" ? "playing" : "paused"}/>
      <div class="caption"><strong>{mode}</strong><span>{mode==="expanded"?"300 × 160 · r45":mode==="hover"?"90 × 30 · r24":mode==="hidden"?"80 × 6 · 顶边提示":"80 × 28 · r24"}</span></div>
    </section>
    <aside>
      <section><h2>预览状态</h2><div class="segmented">{#each ["compact","hover","expanded","hidden"] as item}<button class:active={mode===item} onclick={()=>mode=item as LegacyIslandMode}>{item}</button>{/each}</div></section>
