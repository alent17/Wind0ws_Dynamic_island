<script lang="ts">
  ...
  import { Type,RotateCcw,Trash2,ExternalLink,Monitor,ArrowUp,ArrowRight,ArrowDown,ArrowLeft,RefreshCw,ChevronUp,ChevronDown,Plus,X,Search } from "lucide-svelte";
  import { type IslandEdge, type IslandStyle } from "$lib/islandGeometry";
  ...
  import { DEFAULT_SETTINGS, type AppLanguage, type AppPreferences, type IdleContentItem, type MediaSessionInfo, type MonitorInfo, type WeatherLocationCandidate } from "$lib/api/types";

  let settings=$state<AppPreferences>({...DEFAULT_SETTINGS}); let monitors=$state<MonitorInfo[]>([]); let cacheMessage=$state("");
...
<main>
  <header><div><h1>Isle Studio</h1><p>{t("studioIntro")}</p></div></header>
  <div class="workspace">
    <aside>
