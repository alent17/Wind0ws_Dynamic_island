<script lang="ts">
  ...
  import { RotateCcw,Trash2,ExternalLink,Monitor,ArrowUp,ArrowRight,ArrowDown,ArrowLeft,RefreshCw,ChevronUp,ChevronDown,CloudSun,Search } from "lucide-svelte";
...
<main>
  <header class="studio-header">
    ...
    <div class="studio-status" class:browser={!nativeRuntime} aria-live="polite">
      ...
      <span><strong>{nativeRuntime?t("previewReady"):t("browserDemo")}</strong><small>{t("previewLabel")}</small></span>
    </div>
  </header>
  <div class="workspace">
    ...
    <aside>
      <section class="appearance-section">
        <div class="section-heading"><div><span class="section-kicker">{t("previewLabel")}</span><h2>{t("islandLayout")}</h2></div><span class="live-indicator"><i></i>{t("previewReady")}</span></div>
      ...
      <section class="preferences-section"><div class="section-title"><h2>{t("preferences")}</h2><span class="section-note">{t("studioIntro")}</span></div><div class="select-grid"><label class="compact-select-row"><span>{t("language")}</span><select aria-label={t("language")} value={settings.language} onchange={(e)=>setLanguage(e.currentTarget.value as AppLanguage)}><option value="system">{t("systemLanguage")}</option><option value="zh-CN">{t("chinese")}</option><option value="en">{t("english")}</option><option value="ja">{t("japanese")}</option></select></label><label class="compact-select-row"><span>{t("clock")}</span><select aria-label={t("clock")} value={settings.clockTimeZone} onchange={(e)=>updatePreference({clockTimeZone:e.currentTarget.value})}><option value="system">{t("systemLanguage")}</option><option value="Asia/Taipei">Asia/Taipei</option><option value="Asia/Tokyo">Asia/Tokyo</option><option value="America/New_York">America/New_York</option><option value="Europe/London">Europe/London</option><option value="UTC">UTC</option></select></label><label class="compact-select-row"><span>{t("font")}</span><select aria-label={t("font")} value={settings.fontId} onchange={(e)=>setFont(e.currentTarget.value as AppPreferences["fontId"])}>{#each FONT_OPTIONS as font}<option value={font.id}>{font.id==="system"?t("systemDefault"):font.label}</option>{/each}</select></label></div></section>
...
<style>
  ...
  .studio-brand{min-width:0}.studio-wordmark{display:block;margin-bottom:8px;color:#92939b;font-size:10px;font-weight:700;letter-spacing:.16em}.studio-brand h1{margin:0;color:var(--studio-ink);font-size:26px;line-height:1.05;letter-spacing:-.04em}.studio-brand p{max-width:48ch;margin:8px 0 0;color:var(--studio-muted);font-size:12px;line-height:1.5}
  .studio-status{display:flex;align-items:center;gap:9px;flex:none;padding:9px 11px;border:1px solid var(--studio-line);border-radius:12px;background:rgba(255,255,255,.72);color:var(--studio-muted)}.studio-status i,.stage-topline i{width:7px;height:7px;border-radius:50%;background:var(--studio-accent);box-shadow:0 0 0 4px rgba(49,88,200,.11)}.studio-status span{display:flex;flex-direction:column;gap:2px;min-width:0}.studio-status strong{color:var(--studio-ink);font-size:11px;font-weight:600}.studio-status small{max-width:170px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--studio-muted);font-size:9px}.studio-status.browser i{background:#9c9da5;box-shadow:0 0 0 4px rgba(156,157,165,.12)}
  ...
  .stage-topline{position:absolute;z-index:3;top:17px;left:20px;right:20px;display:flex;align-items:center;justify-content:space-between;gap:12px;color:#898a93;font-size:10px;pointer-events:none}.stage-topline span{display:flex;align-items:center;gap:8px;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.stage-topline i{width:5px;height:5px;box-shadow:none;background:#b1b2ba}.stage-topline small{flex:none;font-size:9px;color:#a0a1a8}
  ...
  .stage-caption{position:absolute;z-index:3;left:20px;right:20px;bottom:17px;display:flex;align-items:center;justify-content:center;gap:7px;min-width:0;color:#777780;font-size:10px;pointer-events:none}
  ...
  .section-heading h2{margin:3px 0 0;font-size:16px;letter-spacing:-.01em}
  .section-kicker{display:block;color:#8b8b94;font-size:10px;line-height:1.2}
  .live-indicator{display:flex;align-items:center;gap:6px;color:#777780;font-size:10px;white-space:nowrap}.live-indicator i{width:6px;height:6px;border-radius:50%;background:var(--studio-accent);box-shadow:0 0 0 4px rgba(49,88,200,.11)}
  .section-title{display:flex;align-items:baseline;justify-content:space-between;gap:12px}.section-title h2{margin:0}.section-note{max-width:230px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#92939b;font-size:9px}
  ...
  .segmented button{border:0;background:transparent;color:#66666e;font-size:11px;cursor:pointer;transition:color 140ms ease,background 140ms ease,transform 120ms ease}.segmented button:hover{color:#111113}
  ...
  .field-label:first-of-type{margin-top:0}.field-label small{color:#777780;font-size:10px}
  .edge-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:6px}
  .edge-grid button{min-height:46px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;border:1px solid transparent;border-radius:10px;color:#66666e;background:#fff;font-size:11px;cursor:pointer;transition:color 140ms ease,background 140ms ease,transform 120ms ease,border-color 140ms ease}.edge-grid button:hover{border-color:#d9dbe1;color:#111113}
  .edge-grid button.active{color:#fff;background:#111113}
  .center-button{width:100%;min-height:36px;margin-top:7px;border:0;border-radius:10px;color:#414148;background:#fff;cursor:pointer}
  .select-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.preferences-section{background:#fff;border-color:#e0e1e6}.preferences-section .section-title{margin-bottom:12px}
  ...
  .compact-select-row span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:12px}.compact-select-row select{min-width:0;max-width:150px;margin-left:auto;border:0;border-radius:8px;padding:6px 8px;background:#fff;font-size:12px;color:#26262b;cursor:pointer}
  .select-row{display:flex;align-items:center;justify-content:space-between;padding:11px 0;border-top:1px solid #e5e5e8}.select-row span{display:flex;align-items:center;gap:8px;font-size:12px}.select-row select{max-width:160px;margin-left:auto;border:0;border-radius:8px;padding:6px 8px;background:#fff}.color-row input[type=color]{width:44px;height:28px;padding:3px;border:1px solid #d7d7dc;border-radius:8px;background:#fff;cursor:pointer}
  ...
  @media(max-width:620px){main{padding:16px 12px 28px}.studio-brand h1{font-size:23px}.studio-brand p{font-size:11px}.stage{height:300px;border-radius:20px}.stage::before{inset:12px;border-radius:15px}.stage-topline{left:15px;right:15px;top:14px}.workspace aside{grid-template-columns:1fr;gap:12px}aside>.appearance-section,aside>.preferences-section,aside>.players-section,aside>.idle-section{grid-column:auto}.select-grid{grid-template-columns:1fr}.appearance-apply-bar{align-items:stretch;flex-direction:column}.apply-actions{justify-content:flex-end}}
  @media(prefers-reduced-motion:reduce){.skeleton-row i{animation:none}.setting-choice,.segmented button,.edge-grid button,.tool-list button,.ordered-row,.icon-button,.ordered-row button,.search-row button,.apply-button,.reset-button{transition:none}}
