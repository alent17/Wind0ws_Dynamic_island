...
describe("island geometry", () => {
  it("keeps the restored compact, hover and expanded dimensions", () => {
    ...
    expect(ISLAND_HOST).toEqual({ width: 300, height: 182, top: 22 });
  });
  ...
  it("sizes and offsets the stable native host for every edge style", () => {
    expect(hostFor("floating", "top")).toEqual({ width: 300, height: 182 });
    expect(hostFor("floating", "left")).toEqual({ width: 322, height: 160 });
    expect(hostFor("edge", "bottom")).toEqual({ width: 300, height: 160 });
  ...
  it("places the host along positive and negative-origin monitors", () => {
    ...
    expect(placementFor(monitor, host, "top", 0)).toEqual({ x: -1920, y: -120, width: 300, height: 160 });
    expect(placementFor(monitor, host, "top", 50)).toEqual({ x: -1110, y: -120, width: 300, height: 160 });
    expect(placementFor(monitor, host, "bottom", 100)).toEqual({ x: -300, y: 800, width: 300, height: 160 });
  });
