          ...
          <section class="function-pane">
            ...
            <div class="function-content">
              ...
              {:else if activeTool === "volume"}
                <VolumePanel volume={systemAudio?.volumePercent ?? 0} muted={systemAudio?.muted ?? false} deviceId={systemAudio?.deviceId ?? ""} devices={audioDevices} switchingDevice={audioDeviceSwitching} onVolume={onAudioVolume} onDevice={onAudioDevice} />
              {:else}
...
<style>
  ...
  .clock-dashboard{gap:11px}.clock-readout{position:relative}.clock-greeting{text-transform:lowercase;letter-spacing:.02em;color:rgba(255,255,255,.48)}.clock-main{display:inline-block;font-family:"Fusion Pixel",var(--app-font),sans-serif;font-size:42px;font-weight:400;letter-spacing:-.02em;text-shadow:2px 2px 0 rgba(242,139,49,.34),-1px -1px 0 rgba(255,255,255,.14);transform:rotate(-1.2deg);transform-origin:left center}.clock-main::after{content:"";display:block;width:52%;height:3px;margin-top:5px;margin-left:2px;border-radius:999px;background:#f28b31;opacity:.75;transform:rotate(-1.8deg)}.clock-meta{margin-top:7px;color:rgba(255,255,255,.46)}
